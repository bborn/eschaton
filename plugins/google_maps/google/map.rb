module Google
  ...
  class Map < MapObject
    ...
    def center=(location)
      ...
      script << "#{self.variable}.setCenter(#{location}, #{zoom});"
    end
    
    # TODO - Make a zoom method
    ...
    def add_control(*types)
      types.flatten.each do |type|
        ...
        script << "#{self.variable}.addControl(new #{google_control}());"
      end
    end
    ...
    def add_marker(marker_or_options)
      ...
      return_script do |script|
        ...
        script << "#{self.variable}.addOverlay(marker);"
      end
    end
    ...
    def click(&block)      
      ...
      body = "if(location){
             ...
             "
      self.listen_to :click, :with => arguments, :on => self.variable, 
                             :body => body
    end
    
    # Opens a information window on the map at the given location
    # :at  - A hash representing the location or a Location object where the infowindow must be placed.
    # :url - URL is generated by rails #url_for. Supports standard url arguments
    def open_info_window(options)
      ...
      if options[:url]      
        url = self.context.url_for(options[:url]).to_s
        parse_url_for_javascript url
      
        %{
          jQuery.get('#{url}', function(data) {          
            #{self.variable}.openInfoWindow(#{at}, data);
          });
        }
      else
        "#{self.variable}.openInfoWindow(#{at}, #{options[:content]});"
      end
    end
    
    def close_info_window
      script << "#{self.variable}.closeInfoWindow();"
    end
